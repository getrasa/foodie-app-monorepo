...
export const LoginPage = () => {
	...
	return (
		<div style={{ minHeight: "100vh", display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1.05fr", background: "var(--fb-cream)", fontFamily: "var(--fb-sans)", color: "var(--fb-ink)" }}>
			{/* ─── LEFT: Form ─── */}
			<div style={{ display: "flex", flexDirection: "column", padding: isMobile ? "24px 20px" : "32px 48px", minHeight: "100vh" }}>
				...
				<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
					...
					<div style={{ fontSize: isMobile ? 12.5 : 13.5, color: "var(--fb-ink-soft)", textAlign: "right" }}>
						...
						<a href="/signup" style={{ color: "var(--fb-ink)", fontWeight: 500, marginLeft: isMobile ? 0 : 6, textDecoration: "none", whiteSpace: "nowrap" }}>Zacznij za darmo →</a>
					</div>
				</div>

				{/* Form */}
				<div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: isMobile ? "28px 0" : "48px 0" }}>
					<form onSubmit={(e) => void handleSubmit(e)} style={{ width: "100%", maxWidth: 380 }}>
						<div style={{ fontFamily: "var(--fb-mono)", fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--fb-ink-mute)" }}>Witaj ponownie</div>
						<h1 style={{ fontFamily: "var(--fb-serif)", fontStyle: "italic", fontSize: isMobile ? 40 : 52, lineHeight: 1.02, letterSpacing: "-0.025em", margin: "12px 0 10px", fontWeight: 400 }}>Buongiorno.</h1>
						<p style={{ fontSize: isMobile ? 14 : 15, color: "var(--fb-ink-soft)", marginBottom: isMobile ? 24 : 32, lineHeight: 1.55 }}>Zaloguj się do swojej restauracji. Goście zostawili kilka słów, kiedy Cię nie było.</p>
						...
						<div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
							<label htmlFor="login-password" style={{ fontFamily: "var(--fb-mono)", fontSize: 10.5, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--fb-ink-mute)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
								<span>Hasło</span>
								<a href="/forgot-password" style={{ fontFamily: "var(--fb-sans)", textTransform: "none", letterSpacing: 0, fontSize: 12, color: "var(--fb-primary)", textDecoration: "none" }}>Nie pamiętasz?</a>
							</label>
							<div style={{ display: "flex", alignItems: "center", gap: 10, padding: "0 14px", borderRadius: 12, background: "#fff", border: "1px solid var(--fb-line)" }}>
								<svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ color: "var(--fb-ink-mute)", flexShrink: 0 }}>
								...
								<input
									...
									style={{ flex: 1, padding: "13px 0", border: "none", background: "transparent", outline: "none", fontFamily: "var(--fb-sans)", fontSize: 15, color: "var(--fb-ink)", minWidth: 0 }}
								/>
								<button type="button" onClick={() => setShowPw(!showPw)} aria-label="Pokaż hasło" style={{ background: "transparent", border: "none", padding: 4, cursor: "pointer", color: "var(--fb-ink-mute)", display: "flex", borderRadius: 6 }}>
						...
						<div style={{ marginTop: 18, padding: "14px 16px", borderRadius: 12, background: "var(--fb-paper)", border: "0.5px solid var(--fb-line)", display: "flex", alignItems: "center", gap: 12 }}>
							...
							<div style={{ flex: 1, fontSize: 12.5, color: "var(--fb-ink-soft)", lineHeight: 1.45 }}>
								<strong style={{ color: "var(--fb-ink)", fontWeight: 500 }}>Wolisz magiczny link?</strong><br />
								<a href="/forgot-password" style={{ color: "var(--fb-primary)", fontWeight: 500, textDecoration: "none" }}>Wyślij mi jednorazowy link do logowania →</a>
							</div>
						</div>
					</form>
				</div>
				...
				{/* Footer */}
				<div style={{ display: "flex", flexWrap: "wrap", gap: 10, justifyContent: "space-between", alignItems: "center", fontSize: 12, color: "var(--fb-ink-mute)", paddingTop: isMobile ? 20 : 32 }}>
					<div>© 2026 Jak Było, Inc.</div>
					<div style={{ display: "flex", gap: isMobile ? 14 : 18 }}>
						<a href="#" style={{ color: "inherit", textDecoration: "none" }}>Prywatność</a>
						<a href="#" style={{ color: "inherit", textDecoration: "none" }}>Regulamin</a>
						<a href="#" style={{ color: "inherit", textDecoration: "none" }}>Pomoc</a>
					</div>
				</div>
			</div>

			{/* ─── RIGHT: Hospitality panel (desktop only) ─── */}
			{!isMobile && (
			<div style={{ position: "relative", overflow: "hidden", background: "var(--fb-paper)", padding: 24, display: "flex", flexDirection: "column", gap: 24 }}>
				...
				<div style={{ position: "relative", flex: 1, minHeight: 0, borderRadius: 18, overflow: "hidden", background: "linear-gradient(180deg, rgba(31,26,21,0) 55%, rgba(31,26,21,0.55) 100%), url('https://images.unsplash.com/photo-1559339352-11d035aa65de?auto=format&fit=crop&w=1400&q=80') center/cover", boxShadow: "0 30px 60px -30px rgba(31,26,21,0.35)" }}>
					...
					<div style={{ position: "absolute", left: 24, top: 24, width: 200, padding: "14px 16px", background: "var(--fb-ink)", color: "var(--fb-cream)", borderRadius: 14, transform: "rotate(-3deg)", boxShadow: "0 20px 40px -12px rgba(31,26,21,0.5)", zIndex: 2 }}>
						...
						<div style={{ fontSize: 10.5, opacity: 0.7, marginTop: 2 }}>następna wizyta · na miejscu</div>
